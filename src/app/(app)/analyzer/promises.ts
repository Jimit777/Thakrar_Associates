 ...
 * one of the cheaper actions in the app: the input is a few thousand tokens
...
export async function generatePromises(
  ...
  try {
    const content = await generateStructured({
      ...
      // No search on purpose: everything needed is below, and search would
      // only invite a claim that can't be traced back to the user's figures.
      prompt: `Check what ${stock.symbol}'s management guided to against what the figures show.\n\n${context}`,
      schema: PromisesSchema,
      thinking: "medium",
